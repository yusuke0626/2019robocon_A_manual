...
int main(void){
	...
	ms.send(TAPE_LED_ARD_NUM,1000,0);
	...
	UPDATELOOP(controller, !(controller.button(RPDS3::START) && controller.button(RPDS3::RIGHT))){
		ms.send(TAPE_LED_ARD_NUM,40,100);
...
//			std::cout << limit_emergency_flag << std::endl;

			if(limit_emergency_flag == true){
				pochama_limit_y_front = false;
				pochama_limit_y_back  = false;
				pochama_limit_z_up    = false;
				pochama_limit_z_down  = false;
				t_arm_limit_right_up  = false;
				t_arm_limit_right_down= false;
				t_arm_limit_left_up   = false;
				t_arm_limit_left_down = false;
			}else{
			...
			std::cout << pochama_limit_y_front << std::endl;
		        std::cout << pochama_limit_y_back  << std::endl;
			std::cout << pochama_limit_z_up    << std::endl;
			std::cout << pochama_limit_z_down  << std::endl;
				/*t_arm_limit_right_up  = gpioRead(RIGHT_UP_T_ARM_LIMIT);  
				t_arm_limit_right_down= gpioRead(RIGHT_DOWN_T_ARM_LIMIT);
				t_arm_limit_left_up   = gpioRead(LEFT_UP_T_ARM_LIMIT);
				t_arm_limit_left_down = gpioRead(LEFT_DOWN_T_ARM_LIMIT);
*/
	...
	gpioWrite(27,false);
	ms.send(255,255,0);
	ms.send(TAPE_LED_ARD_NUM,1000,10000000000000);
	gpioWrite(13,false);
	return 0;
}
